...
import cn.northpark.jeyy.module.Destroyable;
 ...
 * 出于效率的考虑，我们缓存所有来自 IoC 的 Controller 实例，无论其在 IoC 中配置为 Singleton 还是 Prototype 类型。
...
public class GuiceContainerFactory implements ContainerFactory {
    ...
    /** 
	 * 取得所有的实例
     * @see cn.northpark.jeyy.container.ContainerFactory#findAllBeans()
     */
    ...
    /**
     * 初始化
     * @see cn.northpark.jeyy.container.ContainerFactory#init(cn.northpark.jeyy.config.Config)
     */
    ...
    public void destroy() {
        ...
        for (Object bean : beans) {
        	//如果类实现了Destroyable接口的话,那么就销毁,否则什么也不做,这个算是观察者模式吧??
